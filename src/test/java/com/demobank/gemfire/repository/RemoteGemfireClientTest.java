...
public class RemoteGemfireClientTest {
    ...
    public void getTransactionsForGivenCriteria() {
        ...
        TransactionSearchCriteria<Transaction> transactionSearchCriteria
        ...
        List<Page<PdxInstance>> allPages = getAllPages(gemfireClient, transactionSearchCriteria);

        assertEquals(20, allPages.size());

        assertAllPagesAreOfSize(transactionSearchCriteria.getRecordsPerPage(), allPages);
    ...
    private List<Page<PdxInstance>> getAllPages(GemfireClient gemfireClient, TransactionSearchCriteria<Transaction> transactionSearchCriteria) {
        List<Page<PdxInstance>> pageList = new ArrayList();
        Page<PdxInstance> startingPage = gemfireClient.getTransactions(transactionSearchCriteria);
        while (startingPage.getResults().size() >= transactionSearchCriteria.getRecordsPerPage()) {
            TransactionSearchCriteria nextPageCriteria = transactionSearchCriteria.nextPage((Transaction) ((PdxInstance)startingPage.getLastRecord()).getObject());
    ...
    private void assertPageSequence(List<Page<PdxInstance>> pageList) {
        ...
        for (int i = 1; i < pageList.size(); i++) {
            ...
            assertTrue(getAmount(nextPage.firstRecord()).compareTo(lastPageLastAmount) <= 0); //amount sorted in descending order.
